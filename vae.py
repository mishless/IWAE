...
history = vae.fit(x_train, np.rollaxis(np.tile(x_train, reps=(k, 1, 1)), axis=1), batch_size=m, epochs=epochs, validation_data = (x_test, np.rollaxis(np.tile(x_test, reps=(k, 1, 1)), axis=1)))
with open('mnist-history-vae.pickle', 'wb') as file_pi:
    pickle.dump(history.history, file_pi)
vae.save('mnist-vae.h5')
encoder.save('mnist-encoder-vae.h5')
decoder.save('mnist-decoder-vae.h5')
